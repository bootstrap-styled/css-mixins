(function (global, factory) {
	typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports, require('color'), require('bootstrap-styled-utils')) :
	typeof define === 'function' && define.amd ? define(['exports', 'color', 'bootstrap-styled-utils'], factory) :
	(factory((global['bootstrap-styled-mixins'] = {}),global.color,global['bootstrap-styled-utils']));
}(this, (function (exports,color,bootstrapStyledUtils) { 'use strict';
...
function breakpointMax(name) {
  ...
  if (next) {
    var min = bootstrapStyledUtils.unitUtils.rmUnit(breakpointMin(next, breakpoints), bootstrapStyledUtils.unitUtils.UNIT.PX);
    return (min - 1).toString() + bootstrapStyledUtils.unitUtils.UNIT.PX;
  }
  return null;
}
...
function customForms() {
  ...
  var selectBorderWidth = bootstrapStyledUtils.unitUtils.math.multiply($borderWidth, 2);
  var customSelectPaddingRight = bootstrapStyledUtils.unitUtils.math.addition($customSelectPaddingY, $customSelectIndicatorPadding);
  var lineHeightBaseMinusCustomControlIndicatorSize = bootstrapStyledUtils.unitUtils.math.subtract($lineHeightBase, $customControlIndicatorSize);
...
function makeContainer() {
  ...
  if (enableGridClasses) {
    ...
    Object.keys(gridGutterWidths).forEach(function (breakpoint) {
      ...
      var column = mediaBreakpointUp(breakpoint, gutter, '\n        padding-right: ' + bootstrapStyledUtils.unitUtils.rmUnit(gutter) / 2 + bootstrapStyledUtils.unitUtils.detectUnit(gutter) + ';\n        padding-left:  ' + bootstrapStyledUtils.unitUtils.rmUnit(gutter) / 2 + bootstrapStyledUtils.unitUtils.detectUnit(gutter) + ';\n      ');
      columns.push(column);
    });
    return '\n      position: relative;\n      margin-left: auto;\n      margin-right: auto;    \n      ' + columns.join('\n') + '\n    ';
  }
...
function makeGutters() {
  ...
  Object.keys(gridGutterWidths).forEach(function (breakpoint) {
    ...
    gutterValue = '' + bootstrapStyledUtils.unitUtils.rmUnit(gutterValue) / 2 + bootstrapStyledUtils.unitUtils.detectUnit(gutterValue);
...
function makeRow() {
  ...
  if (enableGridClasses) {
    ...
    Object.keys(gridGutterWidths).forEach(function (breakpoint) {
      var gutter = gridGutterWidths[breakpoint];
      gutter = '' + bootstrapStyledUtils.unitUtils.rmUnit(gutter) / -2 + bootstrapStyledUtils.unitUtils.detectUnit(gutter);
      var row = '\n        margin-right: ' + gutter + ';\n        margin-left:  ' + gutter + ';\n      ';
      rowList.push(row);
    });
    return '\n      display: flex;\n      flex-wrap: wrap;\n      ' + rowList.join('\n') + '\n    ';
...
function makeCol(size) {
  ...
  return '\n    flex: 0 0 ' + bootstrapStyledUtils.unitUtils.toPercent(size, columns) + ';\n    /* Add a \'max-width\' to ensure content within each column does not blow out */\n    /* the width of the column. Applies to IE10+ and Firefox. Chrome and Safari */\n    /* do not appear to require this. */\n    max-width: ' + bootstrapStyledUtils.unitUtils.toPercent(size, columns) + ';\n  ';
}
function makeColOffset($size) {
  var columns = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : defaultProps$13['$grid-columns'];
  return '\n    margin-left: ' + bootstrapStyledUtils.unitUtils.toPercent($size, columns) + ';\n  ';
}
function makeColPush(size) {
  var columns = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : defaultProps$13['$grid-columns'];
  return '\n    left: ' + (size > 0 ? bootstrapStyledUtils.unitUtils.toPercent(size, columns) : 'auto') + ';\n  ';
}
function makeColPull(size) {
  var columns = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : defaultProps$13['$grid-columns'];
  return '\n    right: ' + (size > 0 ? bootstrapStyledUtils.unitUtils.toPercent(size, columns) : 'auto') + ';\n  ';
}
...
function navDivider() {
  ...
  var marginY = '' + bootstrapStyledUtils.unitUtils.rmUnit(spacerY, bootstrapStyledUtils.unitUtils.UNIT.REM) / 2 + bootstrapStyledUtils.unitUtils.UNIT.REM;
...
function navbarVerticalAlign() {
  ...
  var marginTop = '' + (bootstrapStyledUtils.unitUtils.rmUnit(navbarHeight, bootstrapStyledUtils.unitUtils.UNIT.PX) - bootstrapStyledUtils.unitUtils.rmUnit(elementHeight, bootstrapStyledUtils.unitUtils.UNIT.PX)) / 2 + bootstrapStyledUtils.unitUtils.UNIT.PX;
  var marginBottom = '' + (bootstrapStyledUtils.unitUtils.rmUnit(navbarHeight, bootstrapStyledUtils.unitUtils.UNIT.PX) - bootstrapStyledUtils.unitUtils.rmUnit(elementHeight, bootstrapStyledUtils.unitUtils.UNIT.PX)) / 2 + bootstrapStyledUtils.unitUtils.UNIT.PX;
...
function assertAscending(map, mapName) {
  ...
  Object.keys(map).forEach(function (key) {
    var num = map[key];
    if (prevNum == null) {
    } else if (!comparable(bootstrapStyledUtils.unitUtils.rmUnit(prevNum), bootstrapStyledUtils.unitUtils.rmUnit(num))) {
      ...
      asserted = false;
    } else if (bootstrapStyledUtils.unitUtils.rmUnit(prevNum) >= bootstrapStyledUtils.unitUtils.rmUnit(num)) {
      if (process.env.NODE !== 'test') {
...
function assertStartAtZero(map) {
  ...
  var firstValue = bootstrapStyledUtils.unitUtils.rmUnit(values[0]);
  var asserted = true;
  if (firstValue !== 0) {
    if (process.env.NODE !== 'test') {
